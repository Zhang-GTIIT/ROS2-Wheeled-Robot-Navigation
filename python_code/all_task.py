...
def main():
    ...
    # 替换为你的实际文件路径
    lift_5f = '/home/agv/wzb/python_code/nav2_task/my_task.json'
    starbucks = ''
    lift_1f = ''
    infohub = ''

    node.send_goal_from_json(lift_5f)

    # move2panel 

    # press button down

    # go in lift & press botton 1f & go out

    # switch map and icp 1f

    node.send_goal_from_json(starbucks)

    # coffee grasp

    node.send_goal_from_json(lift_1f)

    # move2panel 

    # go in lift & go out

    # switch map and icp 4f

    node.send_goal_from_json(infohub)
