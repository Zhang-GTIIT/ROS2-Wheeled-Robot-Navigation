...
geometry_msgs::msg::TransformStamped toRos2(
    const Rigid3d &T, const Time &time,
    const std::string &frame,
    const std::string &childFrame)
{
  geometry_msgs::msg::TransformStamped transformStamped;
  ...
  // ---------- Translation (force 2D) ----------
  transformStamped.transform.translation.x = T.translation().x();
  transformStamped.transform.translation.y = T.translation().y();
  transformStamped.transform.translation.z = 0.0;   // 🔒 锁死 Z

  // ---------- Rotation (extract yaw only) ----------
  Eigen::Quaterniond q = T.rotation();
  Eigen::Vector3d rpy = q.toRotationMatrix().eulerAngles(0, 1, 2);

  double yaw = rpy[2];

  Eigen::Quaterniond q2d;
  q2d.setIdentity();
  q2d = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ());

  transformStamped.transform.rotation.w = q2d.w();
  transformStamped.transform.rotation.x = q2d.x();
  transformStamped.transform.rotation.y = q2d.y();
  transformStamped.transform.rotation.z = q2d.z();
...
void TfPublisher::publishMapToRangeSensor(const Time &t) {
  ...
  geometry_msgs::msg::TransformStamped transformStamped =
      toRos(mapToLidar, t, "map", "base_laser");
...
void TfPublisher::publishMapToOdom(const Time &t) {
  Rigid3d mapToOdom = frameTracker_->getTransformMapToOdom(t);

  // ---- 强制 2D ----
  Eigen::Vector3d t2d = mapToOdom.translation();
  t2d.z() = 0.0;

  // 用旋转矩阵直接算 yaw（最稳）
  Eigen::Matrix3d R = mapToOdom.rotation().toRotationMatrix();
  ...
  Rigid3d mapToOdom2d(t2d, q2d);

  geometry_msgs::msg::TransformStamped transformStamped =
      toRos(mapToOdom2d, t, "map", "odom");
...
void TfPublisher::odometryCallback(const nav_msgs::msg::Odometry &msg) {
  ...
  if (isProvideOdomFrame_) {
    geometry_msgs ::msg::TransformStamped transformStamped = toRos(
        odomToTracking.transform_, odomToTracking.time_, "odom", "base_link");
    transformStamped.header.stamp = nh_->now();
    tfBroadcaster_->sendTransform(transformStamped);
    ...
    transformStamped = toRos(cameraToLidar, odomToTracking.time_, "base_link",
                             "base_laser");
    transformStamped.header.stamp = nh_->now();
    tfBroadcaster_->sendTransform(transformStamped);
...
void TfPublisher::imuCallback(const sensor_msgs::msg::Imu &msg) {
  ...
  geometry_msgs::msg::TransformStamped transformStamped =
      toRos(lidarToImu, imuReading.time_, "base_laser", "inertial_sensor");
  transformStamped.header.stamp = nh_->now();
  tfBroadcaster_->sendTransform(transformStamped);
